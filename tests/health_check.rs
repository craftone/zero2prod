...
async fn spawn_app() -> TestApp {
    ...
    let server =
        zero2prod::startup::run(listener, connection_pool.clone()).expect("Failed to listen");
...
#[tokio::test]
async fn subscribe_returns_a_400_when_fields_are_present_but_empry() {
    let app = spawn_app().await;
    let client = reqwest::Client::new();
    let test_cases = vec![
